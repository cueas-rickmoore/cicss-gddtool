...
console.log("EVENT :: loading display plugin");
...
var GddChartController = {
    ...
    addSeries: function(data_type, data) {
        ...
        console.log("    display.addSeries " + data_type);
        if (data.length > 0) {
            this.validChart();
            if (this.chart) {
                var exists = (this.drawn.indexOf(data_type) > -1);
                console.log("    " + data_type + " in drawn :: " + exists);
                if (exists) {
                    console.log("    requesting removal of " + data_type + " series from chart");
                ...
                if (this.pending_series.length > 0) { this.drawPending(); }
                 ...
                 console.log("WARNING :: CHART NOT READY ... " + data_type + " wants to be drawn");
    ...
    allDrawn: function() { 
        console.log("DISPLAY :: allDrawn : series = " + this.series_order.length + " : drawn = " + this.drawn.length);
    ...
    changeGddThreshold: function(threshold) { 
        if (threshold != this.gdd_threshold) {
            console.log("DISPLAY :: changeGddThreshold : " + threshold);
    ...
    changeLocation: function(loc_obj) {
        ...
        if (address != this.location) {
            console.log("DISPLAY :: changeLocation : " + address);
    ...
    complete: function(reset) {
        console.log("DISPLAY :: complete : " + this.allDrawn());
    ...
    drawPending: function() {
        if (this.pending_series.length > 0) {
            console.log("DISPLAY :: draw pending series : " + this.pending_series);
            this.validChart();
            var series;
            ...
            console.log("DISPLAY :: pending draws are complete");
            this.complete();
        }
    },
    ...
    execCallback: function(reset) {
        if (typeof this.callback !== 'undefined' && this.callback != null) { 
            console.log("DISPLAY :: finished drawing chart");
    ...
    newChart: function() {
        ...
        console.log("    GDD threshold = " + this.gdd_threshold);
        console.log("    title = " + config.title.text);
        config.subtitle.text = this.subtitle();
        console.log("    subtitle = " + config.subtitle.text);
