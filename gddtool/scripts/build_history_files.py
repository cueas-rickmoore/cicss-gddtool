...
class BuildProcessServer(object):
    ...
    def __init__(self, year, source, region, gdd_thresholds, for_prod_build):
        self.active_script = None
        self.arg_queue = [ ]
        self.for_prod_build = for_prod_build
        self.region = region
        self.source = source
        for threshold in gdd_thresholds:
            self.arg_queue.append( (year, threshold) )
    ...
    def initiateProcess(self):
        ...
        script = 'build_history_file.py '
        script += ' '.join(arguments)
        script += ' -r %s -s %s' % (self.region, self.source)
        if self.for_prod_build: script += ' -p'
...
print 'Initiating history file builds for', year, source, region

server.run()

elapsed_time = elapsedTime(BUILD_START_TIME, True)
print 'Finished history file builds in %s' % elapsed_time
