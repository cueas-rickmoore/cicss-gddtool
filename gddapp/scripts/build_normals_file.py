...
def getTheData(reader, dataset_path, days_in_history):
    ...
    days_in_file = daysInYear(reader.target_year)

    if days_in_file == 365 and days_in_history == 366:
    ...
    if days_in_file == 366 and days_in_history == 365:
    ...
    # same number of days in both years
    return gdd
...
filepath = factory.normalGridFilepath(days_in_history, source, region,
                                       gdd_threshold, coverage)
if verbose: print '\nnormal filepath :', filepath
...
# create the normal file and initialize it's groups/datasets
scopes = factory.getAvailableScopes(days_in_history)
if verbose: print 'scopes :', scopes

reader = factory.getStaticFileReader(source, region)
if verbose: print 'reader :', reader

builder = factory.getNormalsFileBuilder(days_in_history, source, region,
...
for group in factory.config.filetypes.normal50.groups:
    scope = group[1]['path']
    first_year, last_year = scopes[scope]
    span_start = datetime.datetime.now()

    msg = '\ngenerating %d day normals %s GDD extremes for %s scope'
    print msg % (days_in_history, coverage, scope)
    gdd_dict = buildGddArrays(factory, days_in_history, first_year, last_year,
                              source, region, gdd_threshold, coverage)

    # update the GDD datasets in the normal file
    msg = '    updating %s dataset in %d %s GDD normald file' 
    manager = factory.getNormalsFileManager(days_in_history, source, region,
                                            gdd_threshold, coverage, 'a')
    for dataset_name, gdd_grid in gdd_dict.items():
        dataset_path = '%s.%s' % (scope, dataset_name)
        print msg % (dataset_path, days_in_history, gdd_threshold)
        manager.open('a')
        manager.updateDataset(dataset_path, 1, gdd_grid)
        manager.close()

    msg = '....finished %s GDD normald in %s'
